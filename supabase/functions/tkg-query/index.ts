// TKG Query Edge Function - Query knowledge from Qdrant
// Deploy to Supabase Edge Functions
// SECURITY: User can only query their own data (enforced from JWT)
...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        // Parse request
        const { query, limit = 5 } = await req.json() as TKGQueryRequest;
        ...
        // 2. Search Qdrant - SECURITY: Always filter by authenticated user_id
        const searchBody = {
            vector: queryVector,
            limit: limit,
            with_payload: true,
        ...
        const qdrantResponse = await fetch(`${QDRANT_URL}/collections/${QDRANT_COLLECTION}/points/search`, {
            method: 'POST',
            headers: {
        ...
        // 3. Format results (don't expose user_id in response)
        const results = qdrantData.result?.map((point: any) => ({
            id: point.id,
            score: point.score,
            content: point.payload?.content || '',
            node_type: point.payload?.node_type || 'FACT',
